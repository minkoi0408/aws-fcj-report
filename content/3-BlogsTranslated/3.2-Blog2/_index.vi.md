---
title: "Blog 2: Phân tích Log và Xây dựng Allowlist cho DNS Firewall"
weight: 1
chapter: false
pre: " <b> 3.2. </b> "
---
...
* **Xác thực/Ủy quyền:** Được quản lý thông qua **OIDC** sử dụng **Amazon Cognito**.
* **Khử trùng lặp (Deduplication):** Một cơ chế khử trùng lặp tự quản lý bằng **DynamoDB** được triển khai thay vì dựa vào các giới hạn của SNS FIFO (TTL 5 phút, SQS FIFO bắt buộc).

### 3. Staging ER7 Microservice
Chịu trách nhiệm chuyển đổi định dạng dữ liệu:
* A Lambda "trigger" đăng ký vào trung tâm pub/sub, sử dụng **bộ lọc thuộc tính**.
* Sử dụng **AWS Step Functions Express Workflow** để thực hiện logic chuyển đổi (**ER7 → JSON**).
* Quy trình công việc sử dụng hai Lambda: một để sửa lỗi định dạng ER7 (ký tự xuống dòng) và một để phân tích cú pháp.
* Kết quả (hoặc lỗi) được đẩy trở lại trung tâm pub/sub để xử lý ở các bước tiếp theo.
